...
void imprimir(Personagem *personagem){
	printf("\n## %s ## %i ## ", personagem->nome,personagem->altura);
...
Personagem calculaPivo(Celula* i, Celula* j){
	int conta = 0;	
	for(Celula* z = i;z!=j;z=z->prox,conta++);
	Celula* resp = i;
...
void quicksort(Celula* esq, Celula* dir, Fila* lista){
	Celula* i = esq, *j = dir;
	Personagem pivo = calculaPivo(i,j);
	//printf("i = %s, j = %s, pivo = %s\n",i->elemento.nome,j->elemento.nome,pivo.nome);
	while(j!=i->ant &&j->prox!=i && j->prox!=i->ant){
		while(strcmp(i->elemento.corDoCabelo,pivo.corDoCabelo)<0||(( strstr(i->elemento.corDoCabelo,pivo.corDoCabelo)) && (strcmp(i->elemento.nome,pivo.nome)<0))){
			i = i->prox;
		}
		while(strcmp(j->elemento.corDoCabelo,pivo.corDoCabelo)>0||(( strstr(j->elemento.corDoCabelo,pivo.corDoCabelo)) && (strcmp(j->elemento.nome,pivo.nome)>0))){
			j=j->ant;
		}
		if(j->prox!=i&&i->ant!=j&&j->prox!=i->ant){
			swap(i,j);i=i->prox;j=j->ant;
		}
	}
	if(j->prox!=esq&&j!=esq){
		quicksort(esq,j,lista);
	}
	if(i!=dir&&i!=NULL&&i->ant!=dir){
