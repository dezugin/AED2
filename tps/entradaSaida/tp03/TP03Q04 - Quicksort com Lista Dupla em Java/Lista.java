...
public class Lista{
	...
	void tratar(String s)throws Exception{
		...
		void swap (int i1, int i2)throws Exception{
			Celula temp = percorrer(i1);
			Celula temp2 = percorrer(i2);
			Personagem character = temp.getElemento();
			temp.setElemento(temp2.getElemento());
			temp2.setElemento(character);
		...
		public Celula percorrer(int x){
			Celula i = primeiro;
			for(int z = 0;z<x;z++,i=i.prox);
			return i;
			}	
		void quick()throws Exception{
			quicksort(0,getTamanho());
		}
		void quicksort(int esq,int dir)throws Exception{
			int i = esq, j = dir;
			String pivo = percorrer((dir+esq)/2).elemento.getCorDoCabelo();
			String pivo2 = percorrer((dir+esq)/2).elemento.getNome();
			while(i<=j){
				comparacoes++;
				while(percorrer(i).elemento.getCorDoCabelo().compareTo(pivo)<0|| ( ( percorrer(i).elemento.getCorDoCabelo().contains(pivo) )&&( percorrer(i).elemento.getNome().compareTo(pivo2)<0 ) )){
					comparacoes++;
					i++;
				}
				while(percorrer(j).elemento.getCorDoCabelo().compareTo(pivo)>0|| ( ( percorrer(j).elemento.getCorDoCabelo().contains(pivo) )&&( percorrer(j).elemento.getNome().compareTo(pivo2)>0 ) )){
					j--;
					comparacoes++;
				}
				if(i<=j){
	...
	public static void main (String [] args){
		...
		*/	//Para cada linha de entrada, gerando uma de saida contendo o numero de letras maiusculas da entrada
			...
			FileWriter hugoFile;
			hugoFile = new FileWriter("396702_quicksort2.txt");
