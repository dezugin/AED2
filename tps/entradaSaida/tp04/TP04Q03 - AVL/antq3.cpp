...
struct AVL* criarAVL(){
	AVL* A = (struct AVL*)malloc(sizeof(struct AVL));
	A->raiz = NULL;
	return A;
}
...
bool pesquisar(char* elemento, AVL* avl){
	return pesquisar2(avl->raiz,elemento);
}
...
No* inserir2(No* no, Personagem x){
	if(no == NULL){
		no = novoItem(x);
	}else if( strcmp(x.nome, no->elemento.nome)<0){
		no->esq = inserir2(no->esq, x);
	}else if (strcmp(x.nome, no->elemento.nome)>0){
		no->dir = inserir2(no->dir, x);
	}else{
		exit(1);
	}
	no = balancear(no);
...
AVL* inserir(AVL* avl, Personagem x){
	avl->raiz = inserir2(avl->raiz,x);
	return avl;
}
...
int main (){
	...
	for(int i = 0; i < numEntrada; i++){
		...
		inserir(lista,personagem);
	}
	
	for(int j = 0;j<numEntrada;j++){
		printf("%s raiz ",input[j].entradas);
		bool temp = pesquisar(input[j].entradas, lista);
		if(temp){
			printf("SIM");
		}else{
			printf("NÃO");
