...
void inserirFim(Personagem x, Lista * lista){
	...
	lista->len[lista->n] = strlen(x.homeworld);
	lista->n++;
}
...
/*
int calcularMaior(Lista * lista){
	int resp = 0;
		for(int i = 0;i<lista->n;i++){			
			if(strlen(lista->array[i].homeworld)>resp){
				resp = strlen(lista->array[i].homeworld);
			}
		}
	return resp;
}
int converterNumero(char c){
	int resp;
	if(c=='*'){
		resp = 0;
	}else{
...
int * converterArray(Lista * lista,int maior,int * maiorChar){
	Personagem temp[lista->n];	
	int arrayNumerico[lista->n][maior];
	for(int i = 0;i<lista->n;i++){
		temp[i] = lista->array[i];
		if((sizeof(lista->array[i].homeworld)/sizeof(char))<maior){
			for(int j = (sizeof(lista->array[i].homeworld)/sizeof(char));j<maior;j++){
				strcat(temp[i].homeworld,'*');
			}
		}
		for(int k = 0;k<(sizeof(lista->array[i].homeworld)/sizeof(char));k++){
			arrayNumerico[i][k] = converterNumero(temp.homeworld[k]);
			if(*maiorChar < arrayNumerico[i][k]){
				*maiorChar = arrayNumerico[i][k];
			}
		}
	}			
	return arrayNumerico;
}
Personagem getPersonagem(int k, int arrayPosicional[], int maiorChar,Lista * lista){
	Personagem pers;	
	char homeworld[100];
	for(int i = 0;i<maiorChar;i++){
		char z = homeworld[i];
		strcat(homeworld,z);
	}
	for(int j = 0;j<lista->n;j++){
		if(lista->array[j].homeworld){
			pers = lista->array[j];
		}
	}
	return pers;
...
void radixsort(Lista * lista){
	int maior = calcularMaior(lista);
	int maiorChar = 0;
	int arrayNumerico[lista->n][maior] = converterArray(lista,maior,&maiorChar);
	int arrayPosicional[maiorChar+1];
	for(int i = 0;i<lista->n;i++){
		for(int j = 0;j<maior-1;j++){
			arrayPosicional[i][j+1] += arrayNumerico[i][j];
		}
	}
	Personagem array2[100];
	for(int k = 0;k<lista->n;k++){
		for(int l = 0;l<maior;l++){
			
		}
		array2[k] = getPersonagem(k,arrayPosicional,maiorChar,lista);	
	}
	lista->array = array2;
}
*/
int getMaior(Lista * lista){
...
void countSort(Lista * lista, int i){
	int j,count[10] = {0};
	Personagem saida[lista->n];
	for(j=0;j<lista->n;j++){
		count[(lista->array[j].altura/i)%10]++;
	}
	for(j=1;j<10;j++){
		count[j]+=count[j-1];
	}
	for(j = lista->n-1;j>=0;j--){
		saida[count[(lista->array[j].altura/i)%10]-1] = lista->array[j];
		count[(lista->array[j].altura/i)%10]--;
	}
	for(j=0;j<lista->n;j++){
			lista->array[j]=saida[j];
	}
}
void radixSort(Lista * lista){
...
int main (){
	...
	FILE * fp;
	fp = fopen ("395702_radixsort.txt", "w+");
	char str[10];
