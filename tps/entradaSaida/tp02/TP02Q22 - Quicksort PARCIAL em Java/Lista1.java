...
class Time {
	...
	public class Lista{
		...
		void merge(Time [] time,int esquerda, int meio, int direita){
		//achar tamanhos de 2 arrays pra serem juntados
		int n1 = meio - esquerda +1;
		int n2 = direita - meio;
		//criar arrays temporarios
		Time []  esquerdo = new Time[n1];
		Time []  direito = new Time[n2];
		//copiar dados para arrays temporarios
		for(int i = 0; i<n1;++i){
			esquerdo[i] = time[esquerda+i];
			comparacoes++;
		}
		for(int j = 0; j<n2;j++){
			direito[j] = time[meio+1+j];
			comparacoes++;
		}
		...
		while(i<n1&&j<n2){
			comparacoes+=3;
			if(esquerdo[i].getPagina().compareTo(direito[j].getPagina())<0){
				time[k] = esquerdo[i];
				i++;
			}else{
				time[k]=direito[j];
		...
		void sort(Time[] time, int esquerda, int direita){
			if(esquerda<direita){
				comparacoes++;
				//achar o ponto do meio
				int meio = (esquerda+direita)/2;
				//organizar a primeira e segunda metades
				sort(time, esquerda, meio);
				sort(time, meio+1,direita);
				// juntar as metades organizadas
				merge(time, esquerda, meio, direita);
			}
		}
		...
		void quicksort(int esq,int dir, int k){
			int i = esq, j = dir;
			String pivo = array[(dir+esq)/2].getApelido();
			while(i<=j){
				comparacoes++;
				while(array[i].getApelido().compareTo(pivo)<0){
					comparacoes++;
					i++;
				}
				while(array[j].getApelido().compareTo(pivo)>0){
					j--;
					comparacoes++;
				}
				if(i<=j){
