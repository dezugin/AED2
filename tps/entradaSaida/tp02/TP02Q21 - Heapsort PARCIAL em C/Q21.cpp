...
void constroi(Lista * lista){
	for(int j = 2;j<lista->n;j++){	
		for(int i = j;i>1&&lista->array[i].altura>lista->array[i/2].altura ||(lista->array[i].altura == lista->array[i/2].altura && strcmp(lista->array[i].nome,lista->array[i/2].nome)>0);i/=2){
			swap(i,i/2,lista);
			comparacoes+=3;
		}
	}
}
int getMaiorFilho(int i, int dir,Lista * lista){
	int filho;
	if(2*i==dir || lista->array[2*i].altura>lista->array[2*i+1].altura){
		filho = 2*i;
		comparacoes++;
	}else if(lista->array[2*i].altura==lista->array[2*i+1].altura&&strcmp(lista->array[2*i].nome,lista->array[2*i+1].nome)>0){
		filho = 2*i;
		comparacoes+=2;
	}else{
		comparacoes+=2;
...
void refaz(int dir, Lista * lista){
	int i = 1, filho;
	while(i<=dir/2){
		comparacoes++;
		filho = getMaiorFilho(i,dir, lista);
		if(lista->array[i].altura<lista->array[filho].altura){
			comparacoes+=1;
			swap(i, filho,lista);
			i = filho;
		}else if(lista->array[i].altura==lista->array[filho].altura&&strcmp(lista->array[i].nome,lista->array[filho].nome)<0){
			comparacoes+=2;
			swap(i,filho,lista);
			i = filho;
		}else{
...
void heapsortParcial(Lista * lista){
	int dir;
	Personagem x;
	constroi(lista);
	dir = lista->n-1;
